...
const RuuviChart = ({ recurringMeasurements, tagFriendlyName }: Props) => {
  ...
  const averageMeasurements = useSelector((state: RootState) =>
    state.measurements.average
      .filter((m) => m.data.friendlyname === tagFriendlyName)
      .map((m) => m.data)
  );

  let currentTagRecurringData = recurringMeasurements;
  ...
  const calculateHeaderData = (currentData: MeasurementData[]) => {
    ...
    const averageTemp = (
      currentData.map((t) => t.temperature).reduce((a, b) => a + b) / currentData.length
    ).toPrecision(4);
    const averageHum = (
      currentData.map((t) => t.humidity).reduce((a, b) => a + b) / currentData.length
    ).toPrecision(4);
    ...
    const highestTempTime = currentData.find((t) => Number(t.temperature) === highestTemp)
      ?.timestamp;
  ...
  if (timeScale !== 24) {
    const currentTime = new Date(
      Number(currentTagRecurringData[currentTagRecurringData.length - 1].timestamp) * 1000
    );
    const beginTime = currentTime.getTime() / 1000 - timeScale * 3600;
    currentTagRecurringData = currentTagRecurringData.filter(
      (measurement) => Number(measurement.timestamp) > beginTime
    );
  }
    ...
    <ChartWrapper>
      <ChartHeader
        ...
        data={calculateHeaderData(
          selectedView === 'recurring' ? currentTagRecurringData : averageMeasurements
        )}
      ...
      />
      <Chart
        measurements={selectedView === 'recurring' ? currentTagRecurringData : averageMeasurements}
      />
    </ChartWrapper>
  );
};
